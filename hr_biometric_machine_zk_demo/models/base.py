...
class ZK(object):
    ...
    #     self.read_sizes()
    #     attendances = []
    #     attendancedata, size = self.read_with_buffer(const.CMD_ATTLOG_RRQ, const.FCT_ATTLOG)
    #     if self.verbose: print("attendance size {} (= {})".format(size, len(attendancedata)))
    #     if size <= 4:
    #         print("WRN: missing attendance data")
    #         return []
    #     total_size = unpack("I", attendancedata[:4])[0]
    #     self.user_packet_size = total_size / self.users
    #     return attendances
    ...
    def get_size_attendance(self):
        ...
        print(response)
        if response == const.CMD_PREPARE_DATA:
            size = unpack('I', self.__data[4:12])[0]
